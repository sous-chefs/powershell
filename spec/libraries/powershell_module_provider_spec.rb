...
describe 'PowershellModuleProvider' do
  ...
  describe 'load_current_resource:' do
    ...
    it 'current resource is enabled' do
      directory = @new_resource.destination.gsub(/\\/, '/') + @new_resource.package_name
  ...
  describe 'download_extract_module:' do
    before do
      direntries = double
      expect(Dir).to receive(:entries).and_return(direntries)
      expect(direntries).to receive(:find)
    end

    context 'when download_url and target are nil' do
      it 'downloads the package' do
        target = ::File.join(Chef::Config[:file_cache_path], 'testmodule' + '.zip')
        cmd_str = "powershell.exe Invoke-WebRequest testmodule -OutFile #{target}; $shell = new-object -com shell.application;$zip = $shell.NameSpace('#{target.gsub('/', '\\\\')}'); $shell.Namespace('C:\\PROGRAMW6432\\WindowsPowerShell\\Modules\\').copyhere($zip.items(), 0x14);write-host $shell"
        expect(Mixlib::ShellOut).to receive(:new).with(cmd_str).and_return(@ps_cmd)
        expect(@ps_cmd).to receive(:run_command)

        expect(@provider.send(:download_extract_module)).to eq(target)
      end
    end
    ...
    context 'when download_url is provided and target is nil' do
      it 'downloads the package' do
        target = ::File.join(Chef::Config[:file_cache_path], 'testmodule' + '.zip')
        cmd_str = "powershell.exe Invoke-WebRequest https://temp_download.com -OutFile #{target}; $shell = new-object -com shell.application;$zip = $shell.NameSpace('#{target.gsub('/', '\\\\')}'); $shell.Namespace('C:\\PROGRAMW6432\\WindowsPowerShell\\Modules\\').copyhere($zip.items(), 0x14);write-host $shell"

        expect(Mixlib::ShellOut).to receive(:new).with(cmd_str).and_return(@ps_cmd)
        expect(@ps_cmd).to receive(:run_command)

        expect(@provider.send(:download_extract_module, 'https://temp_download.com')).to eq(target)
      end
    end
  ...
  describe 'install_module:' do
    ...
    context 'source is a url' do
      it 'downloads module from source and install' do
        ...
        expect(@provider).to receive(:download_extract_module).and_return(@dir)
