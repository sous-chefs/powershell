...
describe 'powershell::install_nuget_provider_manual' do
  {
    ...
    context "on #{windows_version}" do
      ...
      it 'includes creates directory and places file' do
        expect(chef_run).to create_directory("#{ENV['ProgramW6432']}\\PackageManagement\\ProviderAssemblies")
        expect(chef_run).to create_remote_file_if_missing("#{ENV['ProgramW6432']}\\PackageManagement\\ProviderAssemblies\\nuget-anycpu.exe")
      end
    end
  end
end
